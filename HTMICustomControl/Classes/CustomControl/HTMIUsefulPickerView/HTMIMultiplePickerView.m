...
@interface HTMIMultiplePickerView ()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation HTMIMultiplePickerView
...
- (void)congifTheDefaultValues {
    ...
    [tempDefaultIndexs enumerateObjectsUsingBlock:^(NSNumber   * _Nonnull obj, NSUInteger index, BOOL * _Nonnull stop) {
        ...
        if (defaultIndex < 0 || defaultIndex >= values.count) {
            ...
            //NSAssert(NO, assert);
            ifDebug(^{
                //HTMILogInfo(@"%@", [NSString stringWithFormat:@"设置的第%ld列下标不合法, 将会被设置为0", index]);
            });
...
- (void)dealloc {
//    HTMILogInfo(@"HTMIMultiplePickerView ===== dealloc");
}
