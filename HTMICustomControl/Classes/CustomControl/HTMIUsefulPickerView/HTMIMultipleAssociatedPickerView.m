//
//  HTMIMultipleAssociatedPickerView.m
...
@interface HTMIMultipleAssociatedPickerView ()<UIPickerViewDelegate, UIPickerViewDataSource> {
    HTMIToolBar *_toolBar;
    NSArray *_data;
...
@implementation HTMIMultipleAssociatedPickerView
    ...
    [tempDefaultValues enumerateObjectsUsingBlock:^(NSString   * _Nonnull obj, NSUInteger component, BOOL * _Nonnull stop) {
        ...
        if (component == 0) { // 数组
            ...
            if (![tempArray containsObject:obj]) {
                ifDebug(^{
                    //HTMILogInfo(@"%@", [NSString stringWithFormat:@"设置的第%ld列默认值不合法, 将会被设置为第一个元素", component+1]);
                });
                // 如果这一列设置的默认值不存在
...
- (void)dealloc {
    //    HTMILogInfo(@"HTMIMultipleAssociatedPickerView ===== dealloc");
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    label.font = [UIFont htmi_systemFontOfSize:18.0f];
